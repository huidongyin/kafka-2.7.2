...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    //Kafka 生产者（Producer）JMX MBean 的前缀
    private static final String JMX_PREFIX = "kafka.producer";
    //Sender线程名称前缀
    public static final String NETWORK_THREAD_PREFIX = "kafka-producer-network-thread";
    //Kafka Producer 内部度量指标（metrics）的组名称
    ...
    KafkaProducer(Map<String, Object> configs,
        ...
        try {
            ...
            //6.失败重试的退避时间，默认是100ms。
            long retryBackoffMs = config.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG);
            //7.定义key value 对应的序列化器
            ...
            if (metadata != null) {
                ...
                this.metadata = new ProducerMetadata(retryBackoffMs,
                        ...
                        //Topic最大空闲时间，如果在规定时间没有被访问，将从缓存删除，下次访问强制获取元数据
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            //如果消息追加器的消息批次队列已经满了，需要唤醒sender线程发送消息。
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        //返回当前集群的元数据信息，就是当前内存中的集群元数据信息。
        Cluster cluster = metadata.fetch();
        //如果集群的无效topic列表中包含当前topic，抛异常。
        ...
        //分区数不为空 && （分区号不为空 || 分区号<分区总数）
        if (partitionsCount != null && (partition == null || partition < partitionsCount))
            //返回集群元数据信息，并且不需要等待元数据。
        ...
        do {
            ...
            //当元数据更新请求的响应解析完成后，会唤醒所有等待元数据更新的线程，此时业务线程继续往下执行。
            //获取最新的集群元数据
